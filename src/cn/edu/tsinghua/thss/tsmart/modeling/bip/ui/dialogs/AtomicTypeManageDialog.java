...
public class AtomicTypeManageDialog extends AbstractEditDialog {
    ...
    protected void removeAtomicType() {
        String[] selections = listTypes.getSelection();
        if (selections == null || selections.length == 0) {
            return;
        }
        for (String selection : selections) {
            if (AtomicTypeModel.isRemovable(selection)) {
    ...
    protected void initValues() {
        ...
        for (String name : AtomicTypeModel.getTypes()) {
            listTypes.add(name);
        }
    }
